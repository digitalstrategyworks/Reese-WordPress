...
<div id="content" class="grid_16">
	...
	<div id="singleContent" class="grid_12">
		...
		<div id="rightContent" class="grid_4">
			...
			<?php if ( check_DifferentTypeFacts($post->ID, 'sidebar-gallery') || check_DifferentTypeFacts($post->ID, 'sidebar-gallery-2') ) : ?>
			<div class="storyElement gallery first">
				<h3><img src="<?php bloginfo('template_directory'); ?>/images/icon-photos.png" alt="<?php the_DifferentTypeFacts($post->ID, 'sidebar-gallery-title'); ?>" /><?php the_DifferentTypeFacts($post->ID, 'sidebar-gallery-title'); ?></h3>
				<!-- photo galleries -->
				...
				<?php if ( check_DifferentTypeFacts($post->ID, 'sidebar-gallery-2') ) { ?>
			<div class="storyElement gallery">
					<h3><img src="<?php bloginfo('template_directory'); ?>/images/icon-photos.png" alt="<?php the_DifferentTypeFacts($post->ID, 'sidebar-gallery-title-2'); ?>" /><?php the_DifferentTypeFacts($post->ID, 'sidebar-gallery-title-2'); ?></h3>
					<?php $gallery_meta = check_DifferentTypeFacts($post->ID, 'sidebar-gallery-2');
						$gallery = "[nggallery id=".$gallery_meta."]"; ?>
					<?php $gal = $gallery;
						$gal = do_shortcode($gal);
						echo $gal;
			...
			<div class="storyElement video">	
				<h3><img src="<?php bloginfo('template_directory'); ?>/images/icon-video.png" alt="<?php the_DifferentTypeFacts($post->ID, 'sidebar-video-title'); ?>" /><?php the_DifferentTypeFacts($post->ID, 'sidebar-video-title'); ?></h3>
				<?php the_DifferentTypeFacts($post->ID, 'sidebar-video'); ?>
			</div>
			<?php endif; ?>
			
			<?php if( check_DifferentTypeFacts($post->ID, 'sidebar-video-2') ): ?>
			<div class="storyElement video">	
				<h3><img src="<?php bloginfo('template_directory'); ?>/images/icon-video.png" alt="<?php the_DifferentTypeFacts($post->ID, 'sidebar-video-title-2'); ?>" /><?php the_DifferentTypeFacts($post->ID, 'sidebar-video-title-2'); ?></h3>
				<?php the_DifferentTypeFacts($post->ID, 'sidebar-video-2'); ?>
			</div>			
			<?php endif; ?>
			
			<?php if( check_DifferentTypeFacts($post->ID, 'sidebar-video-3') ): ?>
			<div class="storyElement video">	
				<h3><img src="<?php bloginfo('template_directory'); ?>/images/icon-video.png" alt="<?php the_DifferentTypeFacts($post->ID, 'sidebar-video-title-3'); ?>" /><?php the_DifferentTypeFacts($post->ID, 'sidebar-video-title-3'); ?></h3>
				<?php the_DifferentTypeFacts($post->ID, 'sidebar-video-3'); ?>
			</div>			
			<?php endif; ?>
						
			<?php if ( check_DifferentTypeFacts($post->ID, 'sidebar-audio') || check_DifferentTypeFacts($post->ID, 'sidebar-audio-2') || check_DifferentTypeFacts($post->ID, 'sidebar-audio-3') ) : ?>
			<div class="storyElement audio">	
				<!-- sidebar audio -->
					<h3><img src="<?php bloginfo('template_directory'); ?>/images/icon-audio.png" />Take A Listen</h3>
					<?php $audio_meta = check_DifferentTypeFacts($post->ID, 'sidebar-audio'); ?>
					<?php $audio_meta_2 = check_DifferentTypeFacts($post->ID, 'sidebar-audio-2'); ?>
					<?php $audio_meta_3 = check_DifferentTypeFacts($post->ID, 'sidebar-audio-3'); ?>
					<ul class="playlist">
						<li><a href="<?php echo $audio_meta; ?>"><?php the_DifferentTypeFacts($post->ID, 'sidebar-audio-title'); ?></a></li>
						<?php if($audio_meta_2 != "") { ?><li><a href="<?php echo $audio_meta_2; ?>"><?php the_DifferentTypeFacts($post->ID, 'sidebar-audio-title-2'); ?></a></li>
						<?php } if($audio_meta_3 != "") { ?><li><a href="<?php echo $audio_meta_3; ?>"><?php the_DifferentTypeFacts($post->ID, 'sidebar-audio-title-3'); ?></a></li><?php } ?>
					</ul>
