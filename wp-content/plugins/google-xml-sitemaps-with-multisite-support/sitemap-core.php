...
class GoogleSitemapGenerator {
	...
	*/
	function InitOptions() {
		...
		$this->_options["sm_b_filename"]=$this->SitemapFilename();		//Name of the Sitemap file
	...
	*/
	function LoadOptions() {
		...
		if($storedoptions && is_array($storedoptions)) {
			...
			// force new delivery of automatic filename
			$this->_options["sm_b_filename"]=$this->SitemapFilename();
	...
	*/
	...
	/**
	 * Creates the filename for xml sitemap
	 *
	 * @since 4.0
	 * @access private
	 * @author Mario Kostelac
	*/
	function SitemapFilename() {
		...
		$domain = str_replace( array('http://', '.', '/', '\\'), array('', '-', '-', '-'), $siteurl );
	...
	*/
	...
	/**
	 * Check permissions for 'sitemaps' directory
	 *
	 * @since 4.0
	 * @access private
	 * @author Mario Kostelac
	...
	function CheckPermissions() {
		...
		if ( ! file_exists( $dir ) ) {
			...
			if ( is_writeable( dirname( $dir ) ) ) {
				...
				trigger_error( $dir.' cannot be created. Please create it on your own and make it writeable.' );
			}
	...
	*/
	function GetXmlUrl($forceAuto=false) {
		...
		if(!$forceAuto && $this->GetOption("b_location_mode")=="manual") {
			...
			if ( MULTISITE == true && SUBDOMAIN_INSTALL == false ) {
				
				$root_site = $this->GetRootSite();
				return trailingslashit($root_site). $this->GetOption("b_filename");
			}
			else {
				// subdomain install
	...
	/**
	 * Returns the URL for the root site
	 *
	 * @since 4.0
	 * @access private
	 * @author Mario Kostelac
	 * @return The URL to the root site
	*/
	function GetRootSite() {
		...
		if ( $wpdb->blogid == 1 ) {
			...
			$root = substr(get_bloginfo('url'), 0, strrpos(get_bloginfo('url'), '/'));
			return $root;
		}
